...
def initialize_Vrep_connection(sync, port):
    ...
    # v.simxFinish(-1) # comment this line if run in multiple threads. refer to https://forum.coppeliarobotics.com/viewtopic.php?t=1867
...
def initialize_single_device(clientID):
    ...
    #print("Robot parts in CoppeliaSim:")
    joints = []
    for j,name in enumerate(names):
        joint = Joint(name, handles[j])
        #print(f"\t Handles ID {handles[j]}: {name}")
        joints.append(joint)
    ...
    # create tip object
    rt_code, tip_handle = v.simxGetObjectHandle(clientID,'tip',v.simx_opmode_blocking)
    tip = SceneObject('tip',tip_handle,None)
      
    return (device,tip,target)
