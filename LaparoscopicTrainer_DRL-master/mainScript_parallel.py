...
def argparser():
    ...
    """savedir_default = 'D:/Laparoscopic/Model-based PPO/trained_models/' \
                      + str(dt.datetime.now()).replace(':','_')\
                      + '/"""
    savedir_default = f"test_{dt.datetime.now()}"
    ...
    parser.add_argument('--ep_len', type=float, default=5)
    parser.add_argument('--imag_step', type=int, default=20)
    parser.add_argument('--imag_time', type=int, default=8)
    parser.add_argument('--polyak', type=float, default=0.995)
    
    parser.add_argument('--use_gpu', action='store_true')
    parser.add_argument('--port', nargs='+', type=int, default = [20000]) #port to connect to vrep
...
def main(args_dict):
    ...
    buffer = replay_buffer(sample_rate = 1/5)
...
##                print(len(kl_div), mean_div)
                ...
                #early stopping
                if (mean_div > max(100 / ((iteration + args_dict['current_iter']\
                                           + 1)*8), 0.01)):
                    break
